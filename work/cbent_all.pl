...
sub run_dir
{
	...
	if(-d "c:\\cbwork\\xml\\$TXvol")	# 跳過不存在的冊數
	{
		print "-"x70 . "\n$TXvol\n" . "-"x70 . "\n";
...
sub show_top_frame
{
	# 最上方的說明
	$fmTop->Label(
		-text => "◆ CBENT_ALL ◆　由 XML 及缺字資料庫產生 ent 檔",
		)->pack(
			-side => 'left',
			-padx => 10,			# 組件左右留空大小 10
			-pady => 10,			# 組件上下留空大小 10
...
　　本程式是呼叫 c:\cbwork\work\bin\cbent.pl ，程式目的是由 XML 檔及
　　缺字資料庫產生各 XML 檔案所搭配的 ent 檔。

【參數說明】

　　參數在程式中間左方，有大正、卍續、嘉興、正史、藏外各組，用法相同。此處以大正藏為例，
　　參數有三種輸入法：
...
　　本程式會將使用過的參數及上次最後執行的參數存在 cbent_all.ini 該檔之中，
...
　　2007/06/20 V1.0    第一版
　　2009/03/14 V1.1    加入嘉興藏、正史、藏外的選項
