...
sub start_handler 
{
	...
	if ($debug) { print "[$el]"; }
    #如果是rdg或gloss，pass加一（校勘）之後會在/rdg或/gloss減回來
	$pass++ if $el eq "rdg";
	$pass++ if $el eq "gloss";
	#added，pass加一，added=1，之後會減回來
	...
	if ($el eq "p") {
	  $pRend = "";
	  my $type = $att{"type"};
	  #如果遇到p type inline而且沒有結尾符號，則加上句號
	  if (lc($att{"type"}) eq "inline") {
	     if ($text !~ /(║|　|。)$/ and $pass == 0) { $text .= "。";print STDERR "lb=$lb";getc; }
      }
      #如果是p type winline，直接加上句號，然後計算空白
      ...
      #如果是p type dharani而且沒有結尾符號，則加上句號
      ...
      #如果是p type w，那麼
         ...
         $pRend =~ s/($utf8)/$utf8out{$1}/g;
		  if ($pRend eq "") { $pRend = "　"; }
		  #如果text以║結尾，就把$pRend的值給indentOfThisLine
      ...
      if ($heads == 0) {
	      my $parent = lc($p->current_element);
	        ...
	        if ($parent eq "lg") { $addSpace=0; }  
  	         ...
  	         #如果 head的parent是div2而且是經序名，也不空格
  	      ...
  	      #經過以上處理如果還是有addSpace，indent與indentOfThisLine都加上兩個全形空白
  	      if ($addSpace) {
  		    $indentOfThisLine.="　　";
  		    $indent .= "　　" ;
  		  }
  	   }
  	   #處理完head，把heads標上1，免得重複處理，並標出inHead
		$heads++;
		$inHead=1;
	}
	...
	if ($el eq "div1"){
		...
		#遇到div1就指定$div1type
		#如果不是附文，而且是空行，就把indentOfThisLine清除
