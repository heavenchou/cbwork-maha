...
sub default {
	...
	if ($pass==0) {
		if ($$text_ref =~ /^(.*)<\/font>$/) {
			my $s1 = $1;
			if ($debug) { print STDERR "312 text=[$$text_ref]\n"; }
			...
			if ($debug) { print STDERR "322 text=[$$text_ref]\n"; }
		} else { $$text_ref .= $string; }
	}
}
...
{       
	...
	### <item> ###
	if ($el eq "item"){
		if ($att{"n"} ne '') { $$text_ref .= myDecode($att{'n'}); }
		$itemLang = $att{"lang"};
	...
	if ($el eq "lb"){
		$lb = $att{"n"};
		#if ($lb=~/238a/ and $vol eq "T32") { $debug=1; }
...
sub rep{
	...
	if ($got) {
		return $str;
		 ...
		 die "896 Unknkown entity $x!! no_nor=$no_nor\n";
	}
...
sub newPage {
	...
	$heaven2 =~ s/<.*?>//g;
	print "<div class=\"root\" id=\"$heaven1\" title=\"", $heaven2, "\">\n";
	#print '<div class="root" id="', $1, '" title="', myOut($2), '">', "\n";
