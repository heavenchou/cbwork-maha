...
print STDERR "輸出記錄檔: $log_file\n";
print STDERR "來源 XML 檔目錄:$xml_in_dir\n";
print STDERR "缺字圖檔目錄: $gaiji_cb_in_dir\n";
print STDERR "悉曇字圖檔目錄: $sd_gif_in_dir\n";
print STDERR "蘭札字圖檔目錄: $rj_gif_in_dir\n";
print STDERR "內文附圖圖檔目錄: $figure_in_dir\n";
...
if ($vol eq '') {
        ...
        @alldir = grep /^[TXJHWI]/, readdir DIR;
        closedir(DIR);
        foreach $vol (@alldir) {
                do_vol();
        }
...
sub do_vol {
        opendir (INDIR, "$xml_in_dir/$vol") or die;
