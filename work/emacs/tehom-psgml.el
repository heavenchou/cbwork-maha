...
(defun tehom-psgml-insert-controlled-subels (controller newpos)
  ...
  (let*
	...
	(if
	  ...
	  (progn
	    (if
	      ;;Treat text content specially.
	      (string= sub-token-name "#PCDATA")

	      ;;The car of the sub-controller is the text to insert.
	      ;;It would be nice to indent it properly with
	      ;;sgml-indent-line, but that may not be possible while
	      ;;building it.
	      (progn
		(insert "\n" (car sub-controller))
