...
;; Created: 01 Feb 1994
;; $Id: html-helper-mode.el,v 1.1.1.1 2003/05/05 04:07:01 ray Exp $
...
;; Changed default value for html-helper-mode-uses-visual-basic:
;; the default distribution does not offer visual-basic-mode.

;; changed comment matching. Used a function instead of a regexp. Comments 
;; are used to hide script code from browsers that can't handle them, and
;; long comments can overflow the regexp stack.

;; Geeeeeeeeeeeee, now it works even better than sgml/html mode :)

;; Changes in version 3.0.3.5bravo

;; Peter Neergaard <turtle@bu.edu> suggests a correction that's damn right
;; on such regexpe like  \(<%=\s*\) \(<%=\w*\) 
;; 

;; Changes in version 3.0.3.5bravo

;; Peter Neergaard <turtle@bu.edu> suggests a mode to overcome differences
;; between Emacsen when dealing with font-lock faces
;; 
...
(defconst html-helper-mode-version (substring "$Revision: 1.1.1.1 $" 11 15))
...
(defcustom html-helper-mode-uses-JDE t
...
(defun html-helper-insert-new-ASP-buffer-strings ()
  ...
  (cond (html-helper-use-JSP-this-buffer
...
is the mode to run (that's the skeleton to insert in empty files)"
  ...
  (cond (html-helper-mode-run-the-mode
	 ...
	 (make-local-variable 'html-helper-count2)
	 (make-local-variable 'html-helper-mode-local-JSP-not-ASP)
	 (set (make-local-variable 'html-helper-mode-run-the-mode) nil)))
...
(defvar html-helper-font-lock-keywords
  (let (;; Titles and H1's, like function defs.
    ...
    (list				; Avoid use of `keep', since XEmacs will treat it the same as `t'.
     ...
     '("<[?%]=\\([^%?]\\|[?%][^>]\\)*[%?]>" 0 html-helper-builtin-face-name t t)
     '(html-helper-match-asp-php 0 html-helper-builtin-face-name t t)
     ;; This one is to pick
...
;; internal variables
(defvar html-helper-builtin-face-name
...
(defvar html-helper-count2 0 "Counter during comments matching")
...
(defun  html-helper-match-comments (last)
  (cond (html-helper-verbose
	 (message "Fontifying %s... (Comments..%s)" bufname (make-string (incf html-helper-count2) ?.))))
...
(cond (html-helper-mode-uses-JDE
	...
	(jde-entering-java-buffer-hooks
	 (add-hook 'jde-entering-java-buffer-hooks 'html-script-release-setup))
	(t (setq jde-entering-java-buffer-hooks 'html-script-release-setup))))
       (t 
	(cond 
	 (java-mode-hook
