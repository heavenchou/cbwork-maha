...
;; $Id: xslide-font.el,v 1.1.1.1 2003/05/05 04:07:02 ray Exp $
...
(eval-and-compile
  (defvar xsl-font-lock-keywords
	...
	(concat
	 ...
	 "\\([^\"<>]*\\)"
	 "\\(\\"
	 (number-to-string (+ 2 xsl-attributes-alist-regexp-depth))
	 "\\)")
	(list 1 xsl-xslt-alternate-face)
	(list (1+ xsl-attributes-alist-regexp-depth)
	      xsl-xslt-alternate-face)
	(list (+ 3 xsl-attributes-alist-regexp-depth)
	      font-lock-variable-name-face)
	(list (+ 4 xsl-attributes-alist-regexp-depth)
	      xsl-xslt-alternate-face)))
     ;;
	...
	(concat
	 ...
	 "[ \t]*"
	 "\\(=[ \t]*\"\\)"
;; The commented-out code doesn't work here, but similar code works
;; for the xslt attribute regexp.  I don't know why.
;;	 "\\(=[ \t]*\\([\"']\\)\\)"
	 "\\([^\"<>]*\\)"
;;	 "\\(\\"
;;	 (+ 2 xsl-fo-attribute-symbol-alist-regexp-depth)
;;	 "\\)")
	 "\\(\"\\)")
	(list 1 xsl-fo-alternate-face 'append)
	(list (1+ xsl-fo-attribute-symbol-alist-regexp-depth)
