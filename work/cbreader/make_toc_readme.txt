...
bulei1.txt 產生全部部類目錄的來源檔, 這是由惠敏法師的部類列表產生的.
bulei2.txt 產生大正藏各冊的目錄列表.
bulei3.txt 產生卍續藏各冊的目錄列表.
bulei4.txt 產生卍續藏各部的目錄列表.
buleinewsign.txt 產生新式標點的目錄列表.
buleifuyan.txt 產生福嚴三年讀經的目錄列表.
buleilichan.txt 產生杜老師做的禮懺部目錄列表.

以上來源請參考 bulei/bulei_readme.txt

至於大正藏的部則是由各冊來組合的, 所以就不需要產生.

執行前要更新 C:/cbwork/work/bin/sutralst.txt , 這是由 C:/cbwork/work/bin/sutralst.pl 來產生的.
